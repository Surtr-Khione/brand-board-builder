...
// the Builder's 31 sections can hold, in one pass. Two things separate this
// from founder-brief:
...
//
// Create mode runs TWO Sonnet calls in parallel (core identity/voice/visual +
// go-to-market depth) and merges — half the wall time of one giant call and
// each schema stays small enough for the model to fill with care.
// Refine mode (chat turns after the first) sends the current brand + the
// conversation and asks for ONLY the changed fields, merged server-side.
...
1. OWN ONE IDEA. Volvo owns "safety," Apple owns "creative liberation," Patagonia owns "the planet before profit." Every great brand can be reduced to one word or phrase it owns in the customer's head. Decide what this brand owns FIRST — every other field defends that territory.
...
4. CODIFY THE VOICE UNTIL A STRANGER COULD WRITE IT. Mailchimp's and Intercom's voice guides work because they're violation-testable: exact words banned, exact registers set, real do/don't examples. Vague adjectives ("authentic," "engaging") are failure.
...
function systemPrompt(exemplars: ExemplarRow[], role: "core" | "gtm" | "refine"): string {
  const refBlock = exemplars.length
    ...
    : "";
  const roleLine = {
    core: "In this pass you decide the brand's CORE: identity, story, archetype, StoryBrand script, complete voice and writing system, manifesto, color system with light/dark modes, typography, photography, logo direction, motion, sound, sensory physics, and accessibility.",
    gtm: "In this pass you decide the brand's GO-TO-MARKET: audience, three ICPs, customer journey emotions, proof architecture, market position, full offer ladder, competitive frame, content pillars, the five canonical brand stories, and the content calendar. A parallel strategist is deciding the name and visual identity — if no working name was given, refer to the brand generically (\"the brand\", \"we\") so nothing you write depends on the name.",
    refine: "You are refining an EXISTING brand in conversation with its founder. You will receive the full current brand and the conversation. Return ONLY the fields you are changing (plus assistantReply) — leave everything you aren't changing out of the output entirely. Keep every change coherent with the fields you aren't touching, unless the founder asked for a direction change big enough that dependent fields must move together (then change them together).",
  }[role];
...
NAMING RULE:
If a brand name is given, keep it exactly. Otherwise propose ONE strong, ownable name that reads well as a domain-style word and use it consistently.${refBlock}`;
}
...
// Pass A — identity, story, voice, visual system. Everything required: the
// promise of the Generator is a FULL board, so the schema enforces it.
const CORE_SCHEMA = {
  type: "object",
  required: [
    "assistantReply", "brandName", "tagline", "industry", "mission", "vision", "elevator",
    "coreValues", "whyDifferent", "brandPromise",
    "archetype", "secondaryArchetype", "enemy", "victim", "heroStatement", "brandPersonality",
    "storyGuide", "storyProblem", "storyPlan", "storyCTA", "storySuccess", "storyFailure",
    "toneAttributes", "messagingDos", "messagingDonts", "wordsAlways", "wordsNever",
    "voiceInstagram", "voiceLinkedIn", "voiceYouTube", "voiceTikTok", "voiceFacebook", "voiceTwitter",
    "socialPersonality", "emailSignoff",
    "languageRegister", "sentenceStyle", "humorRegister", "personPreference", "readingLevel",
    "numberStyle", "capitalizationStyle", "jargonPolicy", "grammarRules",
    "brandCommandments", "brandNeverDoes", "controversyStance", "crisisVoice", "brandOwnedMoment",
    "primaryColor", "secondaryColor", "accentColor", "colorRationale",
    ...
    "brandSpeed", "brandWeight", "brandTemperature", "brandTexture", "brandDensity", "brandSensoryNotes",
    "moodboardKeywords",
  ],
  properties: {
    assistantReply: { ...S, description: "3-6 sentences to the founder, in plain confident language: the single idea this brand owns, the archetype+enemy choice and why, and which reference-brand patterns informed the build. No markdown headers." },
    // Identity & story
    ...
    // Archetype
    archetype: { ...S, description: "One of the 12 Jungian archetypes: The Innocent, The Explorer, The Sage, The Hero, The Outlaw, The Magician, The Regular Guy/Gal, The Lover, The Jester, The Caregiver, The Creator, The Ruler" },
...
// Pass B — audience, ICPs, journey, proof, market, offer, competitive,
// pillars, story library, calendar.
const GTM_SCHEMA = {
  type: "object",
  required: [
    "audienceAge", "audienceRole", "audiencePains", "audienceGoals", "icps",
    "journeyAwareness", "journeyConsideration", "journeyPurchase",
    "journeyOnboarding", "journeyRetention", "journeyAdvocacy",
    "proofHierarchy", "keyProofStats", "socialProofCriteria", "claimStandards",
  ...
  properties: {
    ...
    // Journey emotions — what the customer FEELS at each stage and the brand's move
    ...
    // Story library — the 5 canonical stories, Setup → Conflict → Resolution → Lesson
    brandStories: {
...
// Refine — every field from both passes, nothing required except the reply.
const REFINE_SCHEMA = {
  type: "object",
  required: ["assistantReply"],
  properties: { ...CORE_SCHEMA.properties, ...GTM_SCHEMA.properties },
};
...
async function callStrategist(system: string, messages: Msg[], schema: object, maxTokens: number) {
  const response = await client.messages.create({
    ...
    tools: [{
      name: "output_brand",
      description: "Output the brand decisions for this pass",
      input_schema: schema,
    }],
...
const CREATE_INSTRUCTIONS = `Build the complete brand from the founder's prompt above.
1. Decide the ONE idea this brand owns, the archetype pairing, and the named enemy first — they are the spine; every field must agree with them.
2. Fill EVERY field in the output schema with finished, specific work — the deliverable is a complete brand operating system, not a questionnaire.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isRefine) {
      // CREATE — two passes in parallel, merged. Core wins any key collision.
      const brief: Msg[] = [{ role: "user", content: `## FOUNDER'S PROMPT\n${lastUser.content.trim()}\n\n${CREATE_INSTRUCTIONS}` }];
      const [core, gtm] = await Promise.all([
        callStrategist(systemPrompt(exemplars, "core"), brief, CORE_SCHEMA, 12000),
        callStrategist(systemPrompt(exemplars, "gtm"), brief, GTM_SCHEMA, 12000),
      ]);
      const { assistantReply, ...coreFields } = core;
      return new Response(
        JSON.stringify({ brand: { ...gtm, ...coreFields }, reply: assistantReply, exemplars: exemplarNames }),
    ...
    const result = await callStrategist(systemPrompt(exemplars, "refine"), convo, REFINE_SCHEMA, 8000);
